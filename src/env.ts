...
export const env = createEnv({
  server: {
    ...
    BASE_URL: z.string().trim().min(1).default("/api/v1").transform(normalizeBaseUrl),
    ...
    CORS_ORIGINS: z.string().default("http://localhost:3000,http://localhost:3001").transform(parseCsv),
    ...
    MONGO_URI: z.string().trim().min(1, "MONGO_URI is required").default("mongodb://127.0.0.1:27017/bright-corner"),
    MONGO_DB_NAME: z.string().trim().min(1).optional(),
    JWT_SECRET: z.string().trim().min(16, "JWT_SECRET must be at least 16 characters").default("change_me_jwt_access_secret"),
    JWT_REFRESH_SECRET: z.string().trim().min(16, "JWT_REFRESH_SECRET must be at least 16 characters").default("change_me_jwt_refresh_secret"),
    COOKIE_SECRET: z.string().trim().min(16, "COOKIE_SECRET must be at least 16 characters").default("change_me_cookie_secret"),
    PASSWORD_PEPPER: z.string().trim().min(1, "PASSWORD_PEPPER is required").default("change_me_password_pepper"),
...
if (env.EMAIL_PROVIDER === "smtp" && env.NODE_ENV === "production") {
  ...
  if (missingSmtpKeys.length > 0) {
    throw new Error(`Missing SMTP configuration: ${missingSmtpKeys.map(([key]) => key).join(", ")}`);
  }
}
...
if (
  ...
  throw new Error("AWS_REGION, AWS_ACCESS_KEY_ID, and AWS_SECRET_ACCESS_KEY are required when AWS_S3_BUCKET is configured.");
}
